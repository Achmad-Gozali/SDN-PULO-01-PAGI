...
import { LayoutDashboard, Newspaper, Image, GraduationCap, LogOut, Menu, X, ChevronRight } from "lucide-react";
          ...
          return (
            <Link key={item.href} href={item.href} onClick={onLinkClick}
              className={cn("flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all",
                isActive ? "bg-blue-600 text-white shadow-lg shadow-blue-600/20" : "text-slate-400 hover:bg-slate-700 hover:text-white")}>
              <item.icon size={18} />
              {item.label}
...
export default function AdminSidebar() {
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <aside className="hidden md:flex flex-col w-64 bg-slate-800 min-h-screen fixed left-0 top-0 z-40 border-r border-slate-700">
      ...
      <button
        ...
        className="md:hidden fixed top-3 left-3 z-50 bg-slate-800 border border-slate-700 p-2 rounded-xl text-white shadow-lg"
      ...
      {isMobileOpen && (
        <div className="md:hidden fixed inset-0 z-40">
          ...
          <aside className="absolute left-0 top-0 w-64 bg-slate-800 h-full border-r border-slate-700">
            <SidebarContent
              pathname={pathname}
